...
param (
    ...
    [string[]]$ExcludeFilePattern = "", #Defines files or paths to exclude from searching
    [string[]]$ValidMatchValues = "",
    [switch]$RecurseDirectory = $false,
    [switch]$IgnoreFilesWithoutMatch = $false
)
...
if ($IsFolder) {
    ...
    if ($RecurseDirectory) {
        $files = Get-ChildItem -File -Filter $FilePattern -Path $FileOrFolderPath -Recurse -Exclude $ExcludeFilePattern
    }
    #Search the directory to get matching files
    else {
        $files = Get-ChildItem -File -Filter $FilePattern -Path $FileOrFolderPath -Exclude $ExcludeFilePattern
    }
